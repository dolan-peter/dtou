...
#system.time({ecolib.dtou<-c_dtouS2B(str,TRUE)[[1]]})
```
...
Here we need to use the `optimizeForSpeed` option because the two strings are very similar and the algorithm will 
system.time({both.dtou<-dtou(c(str,str2),optimizeForSpeed = TRUE)[[1]]})
```
```
And 
